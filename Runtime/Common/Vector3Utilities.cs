...
namespace GameCore
{
    public static class Vector3Utilities
    {
            ...
            int clampSmoothing)
        {
            ...
            var dot = 1f - angle / 180f;

            if (dot > clampWeight) return direction;

            // Clamping the target
            var targetClampMlp = clampWeight > 0 ? Mathf.Clamp(1f - (clampWeight - dot) / (1f - dot), 0f, 1f) : 1f;

            // Calculating the clamp multiplier
            var clampMlp = clampWeight > 0 ? Mathf.Clamp(dot / clampWeight, 0f, 1f) : 1f;

            // Sine smoothing iterations
            for (var i = 0; i < clampSmoothing; i++)
            {
                var sinF = clampMlp * Mathf.PI * 0.5f;
        ...
        /// </summary>
        public static Vector3 ClampDirection(Vector3 direction, Vector3 normalDirection, float clampWeight,
            int clampSmoothing, out float clampValue)
        {
            clampValue = 1f;
            ...
            // Slerping the direction (don't use Lerp here, it breaks it)
            var slerp = clampMlp * targetClampMlp;
            clampValue = 1f - slerp;
        ...
        {
            if (planeNormal == Vector3.up) return new Vector3(point.x, planePosition.y, point.z);
